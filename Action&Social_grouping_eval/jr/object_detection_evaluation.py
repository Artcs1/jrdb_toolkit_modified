...
try:
  import jr.standard_fields as standard_fields
  import jr.label_map_util as label_map_util
  import jr.metrics as metrics
  import jr.per_image_evaluation as per_image_evaluation
except:
...
class ObjectDetectionEvaluator(DetectionEvaluator):
  ...
  def add_single_detected_image_info(self, task, image_id, detections_dict):
    """Adds detections for a single image to be used for evaluation.
    ...
    self._evaluation.add_single_detected_image_info(task,
        image_key=image_id,
  ...
  def evaluate(self):
    ...
    """
    (per_class_ap, mean_ap, precision_ap, recall_ap, per_class_corloc, mean_corloc) = (
...
#        display_name = (
#            self._metric_prefix + 'RecallByCategory/AP@{}IOU/{}'.format(
#                self._matching_iou_threshold,
#                category_index[idx + self._label_id_offset]['name']))
#
#        tam_recall = len(recall_ap)
#        if idx < tam_recall:
#            if recall_ap[idx].shape[0]!=0:
#                pascal_metrics[display_name] = recall_ap[idx][-1]
#        else:
#            pascal_metrics[display_name] = float('nan')
#
...
class PascalDetectionEvaluator(ObjectDetectionEvaluator):
  ...
  def __init__(self, categories,task, matching_iou_threshold=0.5):
...
class ObjectDetectionEvaluation(object):
  ...
  def add_single_detected_image_info(self,
    ...
    # first match, then refine! (grouping and social activity)

    scores, tp_fp_labels = (
        self.per_image_eval.compute_object_detection_metrics(task,
